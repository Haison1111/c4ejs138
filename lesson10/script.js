...
// let FirstName = prompt("nhap vao first_name");
// let LastName = prompt("nhap vao last_name");
// let NewUser = {}
// let Newid = 0
// for ( let i =0;i < users.length;i++){
...
//         console.log(" User nay da co tai khoan")
// }   if( users[i].email !== Email ){
...
for ( let i=0;i< users.length; i++){
     ...
     Usersview[i].ID = users[i].id
     Usersview[i].Name = FullName
     Usersview[i].Email = users[i].email      
} 


// let keyword = prompt("hay nhap keyword") 
// for( let user of Usersview){
...
// 4.Hiển thị danh sách các posts, mỗi post bao gồm ID, title, ngày tạo, họ và tên người tạo.


// let UserPosts = []
// for ( let i = 0; i < posts.length ; i++){
//     UserPosts[i] = {}
//     UserPosts[i].ID = posts[i].id
//     UserPosts[i].Title = posts[i].title
//     UserPosts[i].Posted_at = posts[i].created_at
//     UserPosts[i].pulisher = posts[i].user_id
// } 

// // let pulisher;
// for ( let n = 0; n< UserPosts.length; n++){
//     for (let j = 0; j < Usersview.length;j++){
//         if ( UserPosts[n].pulisher == Usersview[j].ID ){   
//         UserPosts[n].pulisher = Usersview[j].Name
//   } 
//   } 
// } 
...
// 5.nhập vào ID của post. Hãy hiển thị thông tin của 1 post có ID tương ứng: ID, tiêu đề, nội dung, link ảnh, tên người tạo, ngày tạo, ngày sửa đổi

// let Id_number = Number (prompt("nhap vao ID"))
// for( i=0; i < posts.length;i++ ){
//     for ( let j =0; j < Usersview.length;j++){
//         if ( posts[i].user_id == Usersview[j].id)
//         posts[i].pulisher = Usersview[j].Name
//         delete posts[i].user_id
//     }
// }
// for( i = 0; i< posts.length;i++){
//     if ( posts[i].id == Id_number){
//         console.log(posts[i])
//     }
// }

// 6. Nhập vào email của 1 user. Hãy tìm kiếm các posts của user đó và hiển thị dưới dạng danh sách.


let Email_user = prompt("nhap vao email")
let compareID ;
for ( let n = 0; n< Usersview.length;n++){
        if ( Email_user == Usersview[n].Email){ 
             compareID = Usersview[n].ID
            for ( let j = 0 ; j < posts.length; j++){
                if( compareID == posts[j].user_id){
                console.log(posts[j])
            }
        }
}
